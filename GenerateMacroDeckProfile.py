...
import sqlite3
argparser = argparse.ArgumentParser(description='Create Macro Deck Profile from Folders with emotes.')
def MDCreate(cols, rows, folderpath, profilename):
    ...
    profileoutput='{"BufferSize":1048576,"ProfileId": "'+ProfileId+'","DisplayName": "'+profilename+ProfileId+'","Folders": ['
...
if __name__ == '__main__':
    argparser.add_argument('-c', '--columns', dest='cols', help='Number of Columns per Page.', required=True)
    argparser.add_argument('-r', '--rows', dest='rows', help='Number of Rows per Page.', required=True)
    argparser.add_argument('-d', '--dir', dest='folderpath', help="Use Folders inside this direcotry. (default 'Emotes')", default='Emotes')
    argparser.add_argument('-p', '--profilename', dest='profilename', help="Name of the Profile, that will be created. (default 'Twitch-Emotes')", default='Twitch-Emotes')

    args = argparser.parse_args()
    MDCreate(int(args.cols), int(args.rows), args.folderpath, args.profilename)
